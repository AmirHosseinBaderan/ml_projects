...
class HierarchicalClustering:
    ...
    def cluster_distance(self, cluster1, cluster2):
        ...
        for _,p1 in cluster1:
            for _,p2 in cluster2:
    ...
    def fit(self,X,n_clusters=1):
        ...
        for index,point in enumerate(X):
            self.clusters.append([(index,point)])
    ...
    def get_labels(self):
        ...
        labels = [-1] * sum(len(cluster) for cluster in self.clusters)

        for cluster_index, cluster in enumerate(self.clusters):
            for sample_index, point in cluster:
